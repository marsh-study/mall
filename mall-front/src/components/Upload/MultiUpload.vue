...
function handleRemove(removeFile: UploadFile) {
  let filePath = removeFile.url;

  if (filePath) {
    // 获取环境变量中的API基础URL
    const baseUrl = import.meta.env.VITE_APP_API_URL || "http://localhost:9999";

    // 如果filePath包含基础URL，则只保留相对路径部分
    if (filePath.startsWith(baseUrl)) {
      filePath = filePath.substring(baseUrl.length);
    }
...
const previewImg: UploadProps["onPreview"] = (uploadFile) => {
  previewImgUrl.value = getImageUrl(uploadFile.url!);
